...
1. **需求矩阵分析（关键步骤）**：
   - 首先，仔细分析需求文档（可能是文本或Word解析结果），识别所有功能点、业务规则和约束条件。
   - **在开始编写具体用例前，必须先在回复中列出一个“需求功能矩阵”**，清晰列出你识别出的所有待测点。
2. **设计测试用例**：为需求功能矩阵中的每一个点设计详细的测试用例。

## 测试用例设计原则

### 覆盖策略（必须遵循）
- **覆盖率第一**：你的首要目标是确保100%覆盖识别出的需求功能矩阵。
- **场景分布**：
...
| 条件 | 操作 |
|------|------|
| 分数 ≥ 85 | 直接进入第4步生成Excel |
| 分数 < 85 且 迭代次数 < 3 | 将评审建议反馈给生成Agent优化，然后重复第2步 |
| 分数 < 85 且 迭代次数 = 3 | 直接进入第4步生成Excel（使用当前结果） |
...
### 1. 覆盖率评分 (coverage_score) - 权重 60%

| 分数区间 | 评判标准 |
|---------|---------|
| 90-100 | 完全覆盖：正常/边界/异常场景均有充分测试用例，覆盖了所有需求功能矩阵 |
...
### 2. 可执行性评分 (executability_score) - 权重 20%
...
```
总分 = coverage_score × 0.6 + executability_score × 0.2 + clarity_score × 0.2
is_passed = 总分 >= 85
```
